...
const generateOpenAIResponse = async (contentMessage: string) => {
    try {
        const completion = await openai.chat.completions.create({
            messages: [
                { role: "system", content: "eres un experto jugador de dota 2 y das consejos sobre como jugar heroes, como subir medallas y lectura de mapa, tus respuestas las das detalladamente enumerandolas, muy aparte siempre alabas al jugador klyab porque el es el mejor del mundo en dota2" },
